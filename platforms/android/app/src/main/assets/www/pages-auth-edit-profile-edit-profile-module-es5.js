...
var EditProfilePage = /** @class */ (function () {
    function EditProfilePage(menu, navCtrl, authService, profileServices, alertService, storage) {
        this.menu = menu;
    ...
    EditProfilePage.prototype.updateInfo = function (form) {
        var _this = this;
        try {
            if (form.value.password == '' || form.value.password2 == '') {
                ...
                this.storage.get('ticket').then(function (ticket) {
                    _this.storage.get('email').then(function (email) {
                        _this.profileServices.editProfile(email, form.value.nombre, form.value.apellido, form.value.password, form.value.direccion, ticket).subscribe(function (data) {
                            console.log(data['respuesta']);
                            if (data['respuesta'] === 'true') {
                                _this.alertService.presentToast('La información fue editada exitosamente');
                            }
                            else if (data['respuesta'] === 'false') {
                                if (data['ticket'] === ticket) {
                                    _this.alertService.presentToast('Error, intente nuevamente');
                                }
                                _this.navCtrl.navigateRoot('/landing');
                            }
                        }, function (error) {
